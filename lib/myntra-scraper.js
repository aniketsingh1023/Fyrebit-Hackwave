...
// --- Save globally sorted top 5 results into MongoDB Trends ---
async function saveTrends(searchQuery, allProducts) {
  await connectDB()
  ...
  console.log(`✅ Saved top 5 cheapest results for "${searchQuery}" to MongoDB`)
  return updatedTrend
}

// --- Launch Chromium / Puppeteer (works both locally + Vercel) ---
async function launchBrowser() {
  const executablePath =
    process.env.NODE_ENV === "development"
      ? // Local: use installed Chrome/Chromium
...
export async function scrapeAllFashionSites(searchQuery, maxProducts = 20) {
  ...
  // Save cheapest 5 across all platforms into MongoDB
  await saveTrends(searchQuery, allProducts)
